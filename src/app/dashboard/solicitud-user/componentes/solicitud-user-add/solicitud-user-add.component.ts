...
import { ModalHardwareUserComponent } from 'src/app/dashboard/modales/modal-hardware-user/modal-hardware-user.component';
import { SolicitudService } from 'src/app/core/services/solicitud.service';
import { ConfirmModalComponent } from 'src/app/modals/confirm-modal/confirm-modal.component';
import { EnvioMailService } from 'src/app/core/services/envio-mail.service';
import { takeUntil } from 'rxjs/operators';
@Component({
  selector: 'app-solicitud-user-add',
  templateUrl: './solicitud-user-add.component.html',
  styleUrls: ['./solicitud-user-add.component.css']
})
export class SolicitudUserAddComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.mantenedorFormSolUser = this.fb.group({
      ...
      tiposolicitud: ['0', [Validators.required]],
  ...
  tipoSolExistente: boolean = false;
  ...
  guardar() {
    ...
    if (this.mantenedorFormSolUser.get('tiposolicitud')?.value == '' ) {
      this.snackbar.notify('danger', 'Debe seleccionar el tipo de solicitud');
      return;
    }
    ...
    let bodypaso3 = {
      ...
      tiposolicitud: this.mantenedorFormSolUser.get('tiposolicitud')?.value??'',
    ...
    this.dialog
      ...
      .subscribe((res) => {
        if (res) {
          this.preloader = true;
          this.solicitudService.insertasolicitud(bodypaso3).subscribe(
            (data) => {
  ...
  hardwareExistente() {
    let bottonSheet = this._bottomSheet.open(ModalHardwareUserComponent, {
  ...
  enviaMail(data:any) {
    ...
    const datsusuario = data.filter((element: any) => element.idtipo_usuario === 3);
    const admins = data.filter((element: any) => element.idtipo_usuario != 3);

console.log("datsusuario",datsusuario);
console.log("admins",admins);
    let modeloMail = {
      titulo: "Solicitud ingresada",
      subtitulo: "Se ha creado una nueva solicitud",
      textouno: "" + datsusuario[0].nombreUsuario + ", se ha creado una nueva solicitud con el código  "+datsusuario[0].codigosolicitud+".",
    ...
    let bodyetapa4 = admins.map((element: any) => {
      return {
        titulo: "Solicitud ingresada",
        subtitulo: "Se ha ingresado una solicitud al sistema.",

        textouno: element.nombreUsuario + ", se  ha ingresado una nueva solicitud. Por favor, revise su panel de administración.",

        textodos: "Equipo RISK-ID. ",
        linkboton: "",
        asunto: "Solicitud  ingresada",
