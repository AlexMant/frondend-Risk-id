...
@Component({
  selector: 'app-asignacion-user-list',
  templateUrl: './asignacion-user-list.component.html',
  styleUrls: ['./asignacion-user-list.component.css']
})
export class AsignacionUserListComponent implements OnInit , OnDestroy {
  ...
  titulo: string = 'Asignaciones';
  tableDataMaintainer: Array<any>;
  ngOnInit(): void {
    // this.titulo = 'Asignaciones de ' + JSON.parse(localStorage.getItem("userInfo")).nombreUsuario + ' ' + this.vmP.des2;
    this.getData();
  }
  ...
  getData() {
    this.asignacionesHardwareService.getasignacionbyuser(JSON.parse(localStorage.getItem("userInfo")).idusuario).subscribe(
      (data) => {
        console.log(data)
